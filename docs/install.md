...
Run `npm run audit:codex` to capture the live Codex 0.130 native-surface state, including optional release gates that Chedex reports but does not require.
...
Completed and cancelled governed workflows are archived into `~/.codex/workflows/_archive.json` when they leave the active index, and their managed workflow cache directories are removed after the archive snapshot is written. When `Stop` allows the turn to close, it also removes the release-audit and release-delta caches so stale Codex-version guidance is regenerated by the next trusted `SessionStart`.
