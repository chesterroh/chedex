...
- If the plan is meant to flow into `cdx-ralph`, treat it as a governed plan.
...
- If the task is stable-metric optimization over a fixed comparison boundary, prefer `cdx-autoresearch-plan` while the research spec is still forming and `cdx-autoresearch-loop` once the governed loop contract is ready to run.
- If `Non-goals` or `Decision boundaries` are still unclear, run `cdx-clarify` first, or `cdx-deep-interview` when the work needs a durable requirements artifact rather than hand off guesswork.
...
- the intended workflow owner: `cdx-ralph` or `cdx-autoresearch-loop`
- the proof path that the governor will later use to allow completion

If the request is still ambiguous, run `cdx-clarify` first, or `cdx-deep-interview` when a higher-rigor requirements pass is the safer next step.
