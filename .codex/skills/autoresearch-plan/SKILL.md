...
- `autoresearch-plan` owns the normalization step between a loose optimization ask and a runnable experiment contract.
- It should make the loop shape explicit before governed execution begins.
- It should hand off to `autoresearch-loop` only when the experiment can run without re-deriving the core research state.
...
- Route back to `clarify` or `deep-interview` when the real objective, success metric, or decision boundary is still unstable.
- Route to `plan`, `execute`, or `autopilot` instead of forcing a research shape when the work is not actually a metric-driven optimization loop.
...
Treat the plan as ready for `autoresearch-loop` only when all of the following are true:
...
- running the experiment loop directly inside `autoresearch-plan`

## Verification And Completion

- `autoresearch-plan` is complete when `context.md` and `spec.md` are strong enough that `autoresearch-loop` can start without reopening the core framing by default.
...
- Recommended next step: `autoresearch-loop`, `deep-interview`, `clarify`, `plan`, `execute`, or `autopilot`
