...
Use this when the user explicitly wants a rigorous requirements pass, or when lightweight `cdx-clarify` would still leave too much ambiguity around intent, scope, constraints, `Non-goals`, or `Decision boundaries`.
...
- `cdx-deep-interview` owns the high-rigor discovery step between a vague request and an execution-ready requirements artifact.
...
- Do not implement directly inside `cdx-deep-interview`.
...
4. Hand off explicitly.
   - Recommend `cdx-execute`, `cdx-autoresearch-plan`, `cdx-autoresearch-loop`, `cdx-plan`, `cdx-ralph`, or `cdx-autopilot` based on execution weight.
   - Treat `spec.md` as the downstream source of truth unless the user revises it.

## Stop Test

`cdx-deep-interview` is ready to stop when all of the following are true:
...
- ending the interview with a vague `spec.md` that still forces `cdx-plan` or `cdx-execute` to rediscover the requirements

## Verification And Completion

- `cdx-deep-interview` is complete when `context.md`, `interview.md`, and `spec.md` make the problem frame, decisions, and remaining assumptions visible enough for the recommended next lane to proceed safely.
...
- Recommended next step: `cdx-execute`, `cdx-autoresearch-plan`, `cdx-autoresearch-loop`, `cdx-plan`, `cdx-ralph`, or `cdx-autopilot`
