...
- Recommend the lightest next skill that fits the now-grounded task:
  - `cdx-execute` for a small direct change or answer
  - `cdx-deep-interview` for a higher-rigor requirements pass with durable artifacts when scope, decision rights, or hidden assumptions still need active discovery
  - `cdx-autoresearch-plan` for metric-driven optimization work that still needs a grounded research spec
  - `cdx-autoresearch-loop` when the research spec already exists and the governed loop is ready to run
  - `cdx-plan` for scoped but still multi-step work
  - `cdx-ralph` for substantial resumable execution with governed artifacts
  - `cdx-autopilot` for broad iterative work that needs one strict operator-facing shell before handing off to narrower lanes
...
- Acceptance criteria
- Open assumptions
- Recommended next step: `cdx-execute`, `cdx-deep-interview`, `cdx-autoresearch-plan`, `cdx-autoresearch-loop`, `cdx-plan`, `cdx-ralph`, or `cdx-autopilot`
