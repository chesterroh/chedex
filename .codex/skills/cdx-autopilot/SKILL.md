...
`cdx-autopilot` is an operator shell and a governed workflow owner.
...
- Within one workspace, `cdx-autopilot` remains the governed owner by default. Nested `cdx-ralph` or `cdx-ultrawork` execution slices should report through the current `cdx-autopilot` workflow instead of syncing competing governed state.
- If the task resolves into a pure governed research loop, explicitly hand governed ownership to `cdx-autoresearch-loop` instead of keeping `cdx-autopilot` active in the same workspace.
- Governor validation for `cdx-autopilot` is phase-aware: `context.md` is required by `specify`, `spec.md` by `plan`, `plan.md` plus `handoff.json` by `execute`, and `verify.md` by `verify` or terminal states.
- Within the same workspace, syncing a different active governed owner requires an explicit ownership replacement. Nested lanes should report through the current `cdx-autopilot` workflow rather than calling `workflow-sync --replace`.

## Phases

1. Clarify
   - If the request is vague, run `cdx-clarify` to close the biggest ambiguity first.
   - If ambiguity remains high, use `cdx-deep-interview` and reuse its artifacts instead of rediscovering them.
...
4. Execute
   - Drop to `cdx-execute` for small direct work.
   - Hand durable implementation slices to `cdx-ralph` when resumable governed execution is needed, but keep `cdx-autopilot` as the parent owner unless you are explicitly handing off the whole workflow.
   - Use `cdx-autoresearch-plan` while a metric-driven task still needs a grounded research contract.
   - Hand stable governed optimization to `cdx-autoresearch-loop` when the work has clearly become a research loop rather than a broad delivery loop.
   - Let `cdx-ralph` use `cdx-ultrawork` only when lanes are truly independent.
5. Verify
...
- Treat `cdx-autopilot` as the operator-facing owner for broad governed work, not as a substitute runtime beneath Codex.
...
- If the task narrows enough, drop down to `cdx-execute`, hand controlled execution slices to `cdx-ralph`, or explicitly transfer ownership to `cdx-autoresearch-loop` instead of keeping the broader shell in control longer than necessary.
- `cdx-autopilot` is a governed workflow owner; keep `progress.json` current after meaningful steps and require verification before completion.
...
- allowing nested governed lanes in the same workspace to steal runtime ownership from the current `cdx-autopilot` workflow without an explicit handoff
