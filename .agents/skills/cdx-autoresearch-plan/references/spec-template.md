...
Use this template to convert a loose optimization task into a compact research plan that can hand off cleanly to `cdx-autoresearch-loop`.

## Minimum Required Spec

This is the default planning shape for `cdx-autoresearch-plan`.
...
The plan is ready for `cdx-autoresearch-loop` only if you can answer yes to all of these:
